...
output_dir="data/bigbrain"
mkdir -p $output_dir

base_url=https://neuroglancer.humanbrainproject.eu/precomputed/BigBrainRelease.2015/classif_mesh # from configuratioon json
mesh_dir=$(curl "$base_url/info" | jq -r '.mesh')
echo $mesh_dir # prints mesh

# 100 is also fron configuration json
for fragment in $(curl "$base_url/$mesh_dir/100:0" | jq -r '.fragments[]')
do
